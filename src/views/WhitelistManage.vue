  ...
  <div class="whitelist-management">
    <div class="header">
      ...
      <div class="actions">
        ...
        <button class="action-btn add-btn" @click="handleAdd">添加白名单</button>
        ...
        <tbody>
          <tr v-for="username in paginatedUsers" :key="username" class="user-row">
            ...
            <td class="actions-cell">
              <button class="btn btn-remove" @click="removeFromWhitelist(username)">移出白名单</button>
            </td>
          </tr>
          <tr v-if="whiteList.length === 0">
            <td colspan="3" class="empty-message">暂无白名单用户</td>
          </tr>
        </tbody>
      </table>
    </div>
...
export default {
  ...
  computed: {
    ...
    paginatedUsers() {
      ...
      return this.whiteList.slice(start, end)
    }
  },
  async mounted() {
  ...
  methods: {
    async fetchWhitelist() {
      try {
        ...
        if (response.data.status === 200) {
          ...
          console.log('获取白名单成功:', response.data);
        } else if (response.data.status === 401) {
          console.error('401错误：未授权访问白名单');
        } else {
          console.error('获取白名单失败:', response.data.message);
        }
      } catch (error) {
    ...
    async confirmAdd() {
      ...
      try {
        await axios.post('/api/add_whitelist', {
    ...
    async removeFromWhitelist(username) {
      if (!confirm(`确定要将 ${username} 移出白名单吗？`)) {
        return
      }
      
      try {
        await axios.post('/api/remove_whitelist', {
          username: username
        })
    ...
    async handleBatchRemove() {
      ...
      try {
        await axios.post('/api/batch_remove_whitelist', {
