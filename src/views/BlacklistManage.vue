<template>
  <div class="blacklist-management">
    ...
    <!-- 分页控件 -->
    <div class="pagination-container" v-if="blackList.length > 0">
...
export default {
  ...
  computed: {
    ...
    totalPages() {
      const pages = Math.ceil(this.blackList.length / this.pageSize)
      console.log(`BlackList - 总数据: ${this.blackList.length}, 每页: ${this.pageSize}, 总页数: ${pages}`)
      return pages
    },
    paginatedUsers() {
  ...
  methods: {
    ...
    async confirmAdd() {
      ...
      try {
        ...
        for (const name of usernames) {
          logManager.info(`正在添加 ${name} 到黑名单...`);
          const response = await axios.post('/api/add_blacklist_user', { username: name });
          if (response.data && response.data.status === 200) {
            successCount++;
          } else {
            failCount++;
        ...
        if (usernames.length === 1) {
          if (failCount === 0) {
            logManager.success(`成功添加 ${usernames[0]} 到黑名单`);
          }
        } else {
          if (failCount === 0) {
            logManager.success(`成功批量添加 ${successCount} 个用户到黑名单`);
